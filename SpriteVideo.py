...
def ask_path(prompt: str):
    while True:
        ...
        if p.exists():
            return p
        print("No encontré ese archivo. Intenta otra vez.")
...
def slice_horizontal(sheet_img: Image.Image, total_frames: int):
    ...
    if frame_w * total_frames != sw:
        print("⚠️ Advertencia: el ancho del sheet no divide exacto entre los frames; el último puede recortarse.")
    frames = []
    x = 0
...
def main():
    ...
    total_frames = ask_int("frameCount (número total de frames)", min_val=1)
